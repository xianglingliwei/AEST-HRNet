...
"""
这个模型就是原始的hrnet,和原来的hrnet没有任何区别.
为了减少计算量,可以将每个stage的4个block改为2个block，或者可以不改.
如果有必要,还需要再将每个block的NUM_CHANNELS改的更小.
在最后一个stage的每个branch添加一个tam_block,在通道添加了注意力,其实也是时间注意力.为什么这么说？
一个时间的图像是4通道,我将多个时间的图像叠加在一起,比如4个时间的图像,叠加在一起那就是16个通道的图像,同时包含了多通道和多个时间的信息.

This model is the original hrnet, without any difference from the original hrnet.
To reduce the computational load, you can change the 4 blocks in each stage to 2 blocks, or you can leave them unchanged.
If necessary, you also need to further reduce the NUM_CHANNELS in each block.
In the last stage, add a tam_block to each branch, which adds attention to the channels, which is actually temporal
attention. Why do I say that? An image of one time has 4 channels, and I stack images of multiple times together,
for example, if there are images of 4 times, stacking them together results in an image with 16 channels,
simultaneously containing information from multiple channels and multiple times.
"""
...
class tam_block(nn.Module):
    """通道/时间注意力机制:我的数据是4维数据(batch_size,channels/times,width,height).我要在channels/times维度实现注意力机制
    """
...
class HighResolutionNet(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.Sigmoid() # 这里务必要注意,损失函数计算中使用了nn.Sigmoid(),那么这里就不使用;反之亦然,否则会有意料之外的报错.
        )
