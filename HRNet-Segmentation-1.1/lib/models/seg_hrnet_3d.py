...
在最后一个stage的每个branch添加一个tam_block,在时间维度上添加了注意力机制.
从头到尾,不改变帧数(也就是时间数).
在最后阶段，我将输出的3维数组改变为2维数组,直接抛弃帧数.

I simply modified HRNet by replacing the original conv2d with conv3d.
In order to reduce computational complexity, I reduced the number of blocks in each stage from 4 to 2.
If necessary, it might be required to further decrease the NUM_CHANNELS in each block.
In the last stage, I added a TAM_block to each branch, incorporating attention mechanism in the temporal dimension.
Throughout the process, I did not alter the number of frames (i.e., time steps).
In the final stage, I transformed the output from a 3D array to a 2D array, directly discarding the temporal dimension.
"""
...
class tam_block(nn.Module):
    """
    时间注意力机制:我的数据是五维数据(batch_size,channels,time,width,height).我要在time维度实现注意力机制
    Temporal attention mechanism: My data is five-dimensional (batch_size, channels, time, width, height).
    I want to implement an attention mechanism in the time dimen
    """
    def __init__(self, in_planes=4, ratio=4):
        # x的输入格式是:五维数据(batch_size,channels,time,width,height),我的数据的time的数量是4.in_planes就是time
        # The input format of x is a five-dimensional data (batch_size, channels, time, width, height), where the
        # number of time steps in my data is 4. In_planes refers to the time dimension
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        "stem net"
        # self.conv1 = nn.Conv2d(3, 64, kernel_size=(3,3,3), stride=(1,2,2), padding=(1,1,1), bias=False)
        # self.conv1 = nn.Conv3d(4, 64, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1), bias=False)
        # self.conv1 = nn.Conv3d(4, 16, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1), bias=False)
        # self.bn1 = BatchNorm3d(64, momentum=BN_MOMENTUM)
        # self.bn1 = BatchNorm3d(16, momentum=BN_MOMENTUM)
        ...
        # config.MODEL.EXTRA['STAGE2']['NUM_CHANNELS']是一个list.得到什么取决于config中的定义,如果config中
        # config.MODEL.EXTRA['STAGE2']['NUM_CHANNELS']定义了一个值,则得到一个值,如果定义了多个值,则得到一个list.
        block = blocks_dict[self.stage2_cfg['BLOCK']]
        num_channels = [num_channels[i] * block.expansion for i in range(len(num_channels))]
        # BasicBlock的block.expansion为1,因而这里的操作没有改变num_channels中的值.
    ...
    def forward(self, x):
        ...
        # Please specify align_corners=True if the old behavior is desired. See the documentation of nn.Upsample for
        # details.
        ...
        x = torch.squeeze(x, 2)
        # RuntimeError: Expected 4-dimensional input for 4-dimensional weight [1, 270, 3, 3], but got 3-dimensional
        # input of size [270, 256, 256] instead.
